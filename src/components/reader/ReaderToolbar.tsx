...
const ReaderToolbar: React.FC<ReaderToolbarProps> = ({
  ...
  return (
    <div 
      className={`fixed top-5 flex gap-1.5 p-1.5 rounded-md border z-[2000] bg-primary/95 border-border shadow-sm backdrop-blur
...
const ToolbarButton: React.FC<ToolbarButtonProps> = ({ 
  ...
  return (
    <button
      ref={buttonRef}
      className={`w-[30px] h-[30px] flex items-center justify-center cursor-pointer border-none bg-transparent p-0 ml-[1px] rounded opacity-75 
        transition-all duration-150 ease-in-out text-primary/70 hover:opacity-100
        ${isActive ? 'opacity-95 bg-primary/5 text-primary/90' : ''}`}
      onClick={onClick}
      title={title}
    >
      {children}
